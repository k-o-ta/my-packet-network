...
int PrintArp(struct ether_arp *arp, FILE *fp) {
  ...
  static char *op[] = {"undefined",     "ARP request.", "ARP reply.",
                       ...
                       "InARP reply.",  "(ATM)ARM NAK."};
  char buf[80];
  fprintf(fp, "arp---------------\n");
  // #define	ether_arp.arphrd	arphdr.ar_hrd
...
int PrintTcp(struct tcphdr *tcphdr, FILE *fp) {
  ...
  fprintf(fp, "dest=%u,", ntohs(tcphdr->dest));
  fprintf(fp, "seq=%u,", ntohs(tcphdr->seq));
  fprintf(fp, "ack_seq=%u,", ntohs(tcphdr->ack_seq));
...
int PrintUdp(struct udphdr *udphdr, FILE *fp) {
  ...
  fprintf(fp, "dest=%u,", ntohs(udphdr->dest));
